...
> **Absorbs:** `docs/systems/techstack-baseline.md`
...
## CI/CD Pipeline (Design — Not Yet Implemented)

### Current State

Deployments are **manual**:
- Frontend: Vercel auto-deploys from Git push (no gates)
- Backend: `npx convex deploy` run manually
- Mobile: manual build via Xcode / Android Studio
- No automated test gates (0% test coverage)
...
| Unit tests | Vitest (future) | Yes |
| Build | `vite build` | Yes |
| E2E tests | Playwright (future) | No (advisory) |

### Recommended CI Provider

GitHub Actions — standard for Vercel + Convex projects. Example workflow:
