...
Complete reference for Pawtreon's Convex API surface. Documents all queries, mutations, actions, and HTTP endpoints across 19 function files. This spec describes the current callable API — what functions exist, what they accept, what they return, and what authorization they require.
...
Returns: { checkoutUrl: string, donationId: Id<"donations"> }
Errors: Throws if Stripe keys not configured
Side effects: Creates pending donation record, creates Stripe Checkout Session
```
...
| `create` | mutation | Optional | Submit a report. Allows authenticated users (with `reporterId`) or limited anonymous path (with `reporterClerkId` from identity) |
...
| Function | Type | Auth | Description |
|----------|------|------|-------------|
| `getMyPaymentMethods` | query | Required | List saved payment methods for current user |
...
- **Handled events:**
  ...
  - `checkout.session.completed` → calls `internal.donations.confirmCheckoutSessionFromWebhook`
...
| Auth Level | Count | Functions |
|------------|-------|-----------|
| **None (public)** | ~25 | Most `list`/`get` queries across cases, clinics, campaigns, partners, volunteers, activity, comments |
| **Optional** | ~8 | `cases.getUiForLocale`, `home.getLandingFeed`, `community.listThreads/getThread/listComments`, `users.getPublicProfile/getProfileByIdOrMe`, `reports.create` |
| **Required** | ~28 | All mutations except admin/internal: create, update, toggle, submit, etc. |
| **Admin** | ~5 | `clinics.listPendingClaims/reviewClaim`, `reports.listPending/setReviewing/resolve`, `community.lockThread` |
| **Internal** | ~8 | `users.upsert`, 3× `donations.*FromWebhook`, 4× `translate.*` |

### Function Count by File

| File | Queries | Mutations | Actions | Internal | Total |
|------|---------|-----------|---------|----------|-------|
| `cases.ts` | 6 | 3 | 0 | 0 | 9 |
| `donations.ts` | 2 | 3 | 0 | 3 | 8 |
| `users.ts` | 5 | 4 | 0 | 1 | 10 |
| `clinics.ts` | 3 | 2 | 0 | 0 | 5+(1 admin q) |
| `campaigns.ts` | 3 | 0 | 0 | 0 | 3 |
| `community.ts` | 5 | 5 | 0 | 0 | 10 + 4 legacy |
| `volunteers.ts` | 2 | 0 | 0 | 0 | 2 |
| `reports.ts` | 2 | 3 | 0 | 0 | 5 |
| `notifications.ts` | 2 | 3 | 0 | 0 | 5 |
| `achievements.ts` | 1 | 0 | 0 | 0 | 1 |
| `activity.ts` | 4 | 0 | 0 | 0 | 4 |
| `home.ts` | 2 | 0 | 0 | 0 | 2 |
| `social.ts` | 2 | 2 | 0 | 0 | 4 |
| `storage.ts` | 0 | 1 | 0 | 0 | 1 |
| `translate.ts` | 0 | 1 | 0 | 4 | 5 |
| `partners.ts` | 3 | 0 | 0 | 0 | 3 |
| `paymentMethods.ts` | 1 | 2 | 0 | 0 | 3 |
| `petServices.ts` | 1 | 2 | 0 | 0 | 3 |
| `settings.ts` | 0 | 1 | 0 | 0 | 1 |
| `http.ts` | — | — | — | — | 2 HTTP routes |
| **Total** | **~44** | **~32** | **0** | **~8** | **~84 + 2 HTTP** |
