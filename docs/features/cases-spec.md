...
6. **Duplicate detection** — pHash image matching flagged in PRD as P1 (not yet implemented)
...
2. **Duplicate detection** — pHash image matching is P1 in PRD but not implemented. Needs: hash computation on upload, similarity threshold, admin review flow for potential duplicates.
