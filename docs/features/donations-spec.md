...
| **Recurring/monthly** | Future (Phase 2) | Recurring support for rescuers or clinics — not yet built |
...
**Indexes:** `by_user`, `by_case`, `by_campaign_ref`, `by_stripe_checkout_session`, `by_stripe_payment_intent`, `by_idempotency_key`, `by_status`

### `paymentMethods` Table (schema L140-L151)

**Status: Unused placeholder.** No Convex functions reference this table. Fields: userId, type (card/paypal/bank), name, lastFour, expiry, isDefault.

**Recommendation:** Either wire to Stripe Customer saved payment methods (for recurring support in Phase 2) or deprecate table. Do not build custom payment method storage — Stripe handles PCI compliance.
...
|---|---|---|
| `/stripe-webhook` | POST | Stripe event handler (signature-verified) |
...
| D-04 | Event | **When** a Stripe webhook fires with `checkout.session.completed`, the system **shall** verify the Svix/Stripe signature before processing. |
...
3. **Recurring donations** — PRD lists recurring support as Phase 2. Need: Stripe Subscriptions integration, `paymentMethods` table wiring (or Stripe Customer portal), recurring donation management UI.
...
6. **`receiptUrl`** — Currently unused (never populated). Stripe provides receipt URLs via the Charge object. Should the webhook handler extract `charge.receipt_url` and store it?
