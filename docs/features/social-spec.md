...
Define the cross-cutting social interaction primitives that span multiple features: case comments (threaded), user follows, and social signals (likes, saves). These are not standalone features — they enhance cases, profiles, and the feed. This spec documents the shared backend implementations and usage rules.
