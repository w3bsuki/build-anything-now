...
| **Authenticated** | `requireUser(ctx)` | All mutations, user-specific queries: `users.me`, `donations.getMyDonations`, `notifications.*` |
...
1. **Every mutation requires auth** — No anonymous write paths
...
| `reports.create` | 10 reports | 24 hours (UTC day bucket) | `reportRateLimits` table tracks daily counts per `clerkId` |
...
- **Events handled:** `payment_intent.succeeded`, `payment_intent.payment_failed`, `checkout.session.completed`, `invoice.paid`, `customer.subscription.updated`, `customer.subscription.deleted`
