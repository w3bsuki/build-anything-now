...
Complete reference for Pawtreon's Convex data model. Documents all 28 application tables, their fields, indexes, relationships, and usage patterns. This spec is the authoritative schema guide — `convex/schema.ts` is the source of truth for code, this document is the source of truth for intent and constraints.

Absorbs findings from `docs/design-docs/systems/data-model-gap-audit.md` (now resolved).
...
### Table Count: 28 application tables + 1 built-in (`_storage`)
...
| `subscriptionId` | `Id<"subscriptions">` | No | FK → subscriptions table (recurring charges) |
...
| `by_subscription` | `[subscriptionId]` | Subscription donation lookup |
...
| `by_stripe_invoice` | `[stripeInvoiceId]` | Recurring invoice correlation |
| `by_idempotency_key` | `[idempotencyKey]` | Duplicate prevention |
| `by_status` | `[status]` | Status-based queries |
| `by_kind` | `[donationKind]` | One-time vs recurring segmentation |
...
**Purpose:** Tracks recurring support subscriptions (monthly/annual). Stripe webhooks keep subscription status in sync and materialize recurring invoices into `donations` rows.

**Fields:**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `userId` | `Id<"users">` | Yes | FK → subscriber (payer) |
| `targetType` | `"case" \| "user"` | Yes | What is being supported |
| `targetId` | `string` | Yes | Document ID of target |
| `stripeSubscriptionId` | `string` | No | Stripe subscription identifier |
| `stripeCheckoutSessionId` | `string` | No | Stripe checkout session identifier |
| `status` | `"pending" \| "active" \| "past_due" \| "canceled" \| "unpaid"` | Yes | Subscription lifecycle state |
| `amount` | `number` | Yes | Amount per interval |
| `currency` | `string` | Yes | ISO currency code |
| `interval` | `"month" \| "year"` | Yes | Billing interval |
| `createdAt` | `number` | Yes | Creation timestamp |
| `updatedAt` | `number` | No | Last sync/update timestamp |
| `canceledAt` | `number` | No | Cancel timestamp |

**Indexes:** `by_user` [userId], `by_user_status` [userId, status], `by_target` [targetType, targetId], `by_checkout_session` [stripeCheckoutSessionId], `by_stripe_subscription` [stripeSubscriptionId]

**Usage:** `subscriptions.ts` (create/list/cancel, webhook sync), `http.ts` (Stripe webhook handler), `donations.ts` (donationKind/ledger integration).
...
### Gap Audit Resolution (from `docs/design-docs/systems/data-model-gap-audit.md`)
