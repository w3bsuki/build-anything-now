...
// Stripe path: create payment intent + pending donation record
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      return {
        ...
        clientSecret: null,
        paymentIntentId: existing.stripePaymentIntentId ?? null,
        mode: process.env.STRIPE_SECRET_KEY ? "stripe" : "preview",
      };
    }
    ...
    await ctx.db.insert("auditLogs", {
      ...
      action: "donation.payment_intent_created",
