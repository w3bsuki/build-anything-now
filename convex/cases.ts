...
// List cases, returning localized fields for requested locale.
export const listForLocale = query({
  ...
  handler: async (ctx, args) => {
    ...
    return cases.map((c) => {
      ...
      return { ...c, ...localized, lifecycleStage: normalizeLifecycleStage(c.lifecycleStage) };
    });
  },
});
...
export const getForLocale = query({
  ...
  handler: async (ctx, args) => {
    ...
    return { ...caseDoc, ...localized, lifecycleStage: normalizeLifecycleStage(caseDoc.lifecycleStage) };
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    const items = await Promise.all(
      paginatedCases.map(async (c) => {
        const item = await mapCaseForUi(ctx, c, locale, null);
...
// Verification status update (admin/clinic)
export const updateVerificationStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (user.role !== "admin" && user.role !== "clinic") {
      throw new Error("Only clinic/admin can update verification status");
    }
    ...
    await ctx.db.insert("auditLogs", {
      ...
      entityType: "case",
      entityId: String(args.caseId),
      action: "case.verification_updated",
      details: `verificationStatus=${args.verificationStatus}`,
      createdAt: now,
    });
  },
});

// Risk level update (admin only)
export const updateRiskLevel = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("auditLogs", {
      ...
      entityType: "case",
      entityId: String(args.caseId),
      action: "case.risk_level_updated",
      details: `riskLevel=${args.riskLevel}`,
      metadataJson: JSON.stringify({ riskFlags: args.riskFlags ?? [] }),
      createdAt: now,
    });
  },
});
